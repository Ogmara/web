...
- `news_following_empty` translation key was missing from every locale, so
  ...
  text. Added across all 7 languages. Same gap found and fixed on desktop
  (1.76.2), which shares this exact string/behavior.
...
- **Followed topics, channel groups and hidden DMs now sync across nodes, not
  just across devices on the same node.** Needs l2-node 0.125.0+ (which gossips
  the encrypted settings blob on the profile topic) and `@ogmara/sdk` 0.54.0.
  The settings upload now carries a cleartext `updated_at` (the max `updatedAt`
  across the synced objects) as the node's last-writer-wins key, and on
  connecting to a node the client re-uploads its copy once when it is newer than
  the node's (or the node has none) — seeding that node, which then gossips it
  on. Switching home node (failover, best-ping, or by hand) now surfaces the
  same followed topics / groups / hidden conversations instead of a different
  set per node.
...
- News Feed topic groups: creating a group, renaming it, and adding a hashtag to
  ...
  that also render unreliably inside the Tauri webview (desktop). Replaced with
  inline `<input>` editing and a styled floating menu (Add topic / Rename /
  Delete), matching the channel-group sidebar. New groups are created with a
  default name and drop straight into inline rename. (Delete still uses the same
  `confirm()` the channel-group sidebar uses.)
...
- **Profile "Posts" count stuck at 50** for any user with more than 50
  posts. `UserProfileView.tsx` displayed `posts()?.length` — the length of
  the `limit: 50`-capped fetch — instead of the server's real count. Now
  reads the API response's `total` field (fixed server-side in l2-node
  0.122.2, which had the same bug: it echoed back the page length instead
  of computing a real total).
...
- **No way back from a "details" view to wherever you actually opened it
  ...
  results, and more, but once there, desktop had zero way out short of the
  browser's own back button. `ChannelSettingsView.tsx`, `NewsDetailView.tsx`,
  and `DmConversationView.tsx` had a back button, but it always returned to
  one hardcoded parent regardless of where the view was actually opened from
...
- **Reposts rendered as empty cards** in the feed, a repost's own permalink,
  bookmarks, and a user's profile posts — header and action row, no title,
  no body, no link back to what was reposted. A `NewsRepost` payload only
  carries `{original_id, original_author, comment}`; the code decoded it the
  same way as a `NewsPost` (`title`/`content`), which is always empty for
  this type. Requires l2-node 0.122.1, which now enriches repost items with
  `original_*` preview fields — `NewsCard`, `NewsDetailView`, `BookmarkCard`,
  and the profile posts list all gained a quote-card branch that renders the
...
- **Cross-node media retry budget (45s) was sometimes shorter than the
  ...
  realistic chance to land, permanently failing an attachment until the
  user navigated away and back (confirmed live: an image 404'd through the
  full retry window, then decoded fine after switching channels and
  returning — same CID, same key, it just needed more time). Raised the
  budget to 3 minutes and the poll interval to 5s (fewer redundant requests
  over the longer window) in `src/lib/mediaCrypto.ts`.
...
- **React to channel deletion live.** A deleted private channel used to just sit in the sidebar
  forever — "joined channels" is a client-local list with no server resync, and unlike kick/ban
  there was no WS event for deletion at all. New `channel_deleted` WS handler in `Sidebar.tsx`
  (bundled SDK 0.41.0) drops the channel from the local list and bounces out of its view if
  currently open, mirroring the existing kick/ban handling.
...
- **Encrypted public/ReadPublic channels (P4 / protocol §8.1).** New channels are created
  E2E-encrypted (forced on, no toggle) via `encryptionEnabled: true`; the P2 channel-crypto
  machinery is reused. The encrypt/decrypt decision is now driven by the channel's
  `encryption_enabled` flag (`isEncrypted()`), not `channel_type === private`, so legacy
  plaintext channels stay v1 (dual-read).
  - Permissionless key establishment: in a public encrypted channel **any member** may seed/
    cover the epoch key (`canEstablishKey()`), vs mods-only for private; the rotation floor
    applies to private channels only (public has no removal-rotation — anti-bulk-readout, not
    member-confidential).
  - **Auto-join on open** for encrypted public channels — you must be a member to receive the
    epoch key (permissionless join; private channels still use the invite/join flow).
  - Attachments ride as plaintext metadata in encrypted public channels (media-file encryption
    is P5); they remain blocked in **private** channels (IPFS media would leak to non-members).
  - Requires l2-node 0.79.0+ / `@ogmara/sdk` 0.38.0+.
- **Honest-claims encryption indicator** in the channel header (🔒) — "Encrypted" for private,
  "Encrypted · public by design" for public/ReadPublic (per spec §13: public channels are never
  labelled "private"/"confidential"). New `channel_encrypted_private` / `channel_encrypted_public`
...
- **E2E key-recovery vault (P3 / protocol §2.5).** DM `conv_key`s and private-channel
  ...
  - The backup key derives once per session from `signMessage("ogmara-keyvault-v1")`
    (silent for built-in wallets; one popup for Extension/K5), cached in memory with
    concurrent-call dedup so restore + backup share a single popup.
...
- Cleared all `npm audit` findings (build toolchain only — esbuild/vite are
  ...
  vulnerabilities; web + bundled SDK build verified on the patched esbuild.
...
- **ChatView role-fetch loop (client-side DoS).** The per-channel role
  `createEffect` fetched `/channels/{id}/members?limit=200` on a cache miss but
  only populated the cache after the async resolved — so when the effect re-fired
  faster than the fetch completed, every re-fire missed and launched another
  request, flooding the node until the browser ran out of sockets
  (`ERR_INSUFFICIENT_RESOURCES`). Added an in-flight guard (at most one fetch per
  channel+wallet); the first success caches the role and stops the churn.

### Changed

- **Remember reachable nodes** (`getAvailableNodes`): peers the client actually
  reached are persisted to `knownNodes` (capped at 50, FIFO; unreachable nodes
  excluded), so the node picker is never empty on a later open even if live
  discovery transiently fails. Adopts sdk-js >=0.26.1.
...
- **Reverted the 0.42.2 DM poll** — it caused sent messages to duplicate (optimistic
  `local-` copy + polled real copy weren't deduped). Reverting restores correct
  display; cross-node DM delivery is being fixed node-side instead.
- **DM conversation isolation.** Clear per-conversation local message state when the
  ...
  *other* peers) — fixes your sent messages leaking into other conversations.
...
- **Reactions from other users now appear live** (previously only after leaving
  and re-entering the channel). 0.40.0 refetched on a reaction event, but the
  authoritative resource was hidden by the localMessages-first dedup: a message
  received via WS sits in `localMessages` with no reactions and wins the dedup,
  so the refetched reaction count never showed. `allMessages` now overlays the
  resource's `reactions` onto the localMessages copy when present — and because
  the node omits the `reactions` field when empty, this never clobbers an
  optimistic own-reaction before the refetch catches up.
